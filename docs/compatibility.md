...
- **Upgrade constraint (Enterprise only):** agents must be on 1.21.7+ with an
  ...
  affected.
- The old "**v2 catalog**" / resource API was an _experiment_ introduced in
...
_feature gate_ keyed on the detected agent version and edition, not two
...
`ErrUnsupportedFeature` _before_ sending a request that the agent would reject.
